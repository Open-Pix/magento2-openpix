...
class Button extends Action
{
    ...
    public function execute()
    {
        ...
        if ($hasActiveWebhook) {
            $hasActiveWebhookPayload = $this->returnHasActiveWebhookPayload(
                $webhook
            );
        ...
        $this->_helperData->log(
            "OpenPix: User doesn't have connection with api ",
            self::LOG_NAME,
            [
                'app_ID' => $appID,
    ...
    public function createNewWebhok(
        ...
        $this->_helperData->log(
            'OpenPix: Try create new weebhook ',
            self::LOG_NAME,
            $this->_curl->getBody()
        );
    ...
    public function handleError($responseBody, $additionalLogs = [])
    {
        ...
        $errorResponse =
            $responseBody['error'] ?? $responseBody['errors'][0]['message'];
        $this->_helperData->log($message, self::LOG_NAME, [
            $errorResponse,
            $additionalLogs,
        ]);
        return [
    ...
    public function getAppId()
    {
        ...
        if (empty($appID) && !empty($this->_helperData->getAppID())) {
            $appID = trim($this->getRequest()->getParam('app_ID'));
            $this->_helperData->setConfig('app_ID', $appID);
        }
        if (!$appID) {
            $this->_helperData->log(
                'OpenPix: Cannot get app_ID of user',
                self::LOG_NAME
            );
            return false;
        }
